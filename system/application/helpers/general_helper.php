<?php 
	...
	function is_import_data_finish($sub_district_code)
	{
		...
		$CI->db->where('sub_district_code', $sub_district_code);
		$participant_details		=	$CI->db->get('participant_details');
		
		$CI->db->where('PM.sub_district_code', $sub_district_code);
		$CI->db->join('participant_details AS PM', 'PM.participant_id = PD.participant_id');
		$participant_item_details		=	$CI->db->get('participant_item_details AS PD');		
		
		if ($school_master->num_rows() > 0 and $school_details->num_rows() > 0 and $participant_details->num_rows() > 0 and $participant_item_details->num_rows() > 0)
		{
			return TRUE;
		}
		return FALSE;
	...
	function is_define_kalolsavam($sub_district_code)
	{
		$CI =& get_instance();
		$CI->db->where('sub_district_code', $sub_district_code);
		$CI->db->where('venue !=', '');
		...
		$sub_dist_kalolsavam_master		=	$CI->db->get('sub_dist_kalolsavam_master');
