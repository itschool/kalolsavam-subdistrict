...
class Welcome extends Controller {
	...
	function index()
	{
		...
		if($this->session->userdata('USER_TYPE')==3){
			...
			$welcome_label		=	$sub_dist_name.' Subdistrict Kalolsavam 2011 - 2012';
		}
		else if($this->session->userdata('USER_TYPE')==2){
			$dist_code		=	$this->session->userdata('DISTRICT');
			$dist_name		=	get_dist_name($this->session->userdata('DISTRICT'));
			$welcome_label	=	$dist_name.' District Kalolsavam 2011 - 2012';
		}
		else if($this->session->userdata('USER_TYPE') < 2){
			$welcome_label	=	'Kerala Kalolsavam 2011 - 2012';
		}
