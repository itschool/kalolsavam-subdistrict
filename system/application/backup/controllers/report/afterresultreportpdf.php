...
class Afterresultreportpdf extends Controller {
		...
		*/		//echo $festival.'</br>'.$itemcode;
		...
		function total_points_Html()
		{
			...
			$this->Contents['retvalue']= $this->afterresult_model->total_points_Html();
			if(count($this->Contents['retvalue'])>0)
			{
			$content	=	$this->load->view('result/afterresult_report/rpt_total_points_Html',$this->Contents);
			//print_r($retvalue);
			/*$html2pdf = new CI_HTML2PDF('P','A4', 'en');
			$html2pdf->pdf->SetDisplayMode('fullpage');
			$html2pdf->WriteHTML($content, '');
	  ...
	  function schoolhigher_result()
	  {
		...
		{
			$this->Contents=array();
			$retvalue  =$this->afterresult_model->subdistrict_resultall();
			if(count($retvalue)>0){
			
			$this->Contents['retvalue']=$retvalue;
			
			$content	=	$this->load->view('result/afterresult_report/subdistrict_resultall',$this->Contents,true);
			//print_r($retvalue);
			$html2pdf = new CI_HTML2PDF('P','A4', 'en');
			$html2pdf->pdf->SetDisplayMode('fullpage');
			$html2pdf->WriteHTML($content, '');
			$html2pdf->Output('subdistrict_resultall.pdf', 'I');
			}
			else {
				redirect('test/nodata');
