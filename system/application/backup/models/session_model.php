...
class Session_Model extends Model{
	...
	function is_user_logged($redirect = true){
		...
		}else if (!is_import_data_finish ($this->session->userdata('SUB_DISTRICT')) and $this->session->userdata('USER_GROUP')!='W'){
			header ( "Location: ". base_url(). 'import/' );
		}else if (!is_define_kalolsavam ($this->session->userdata('SUB_DISTRICT')) and $this->session->userdata('USER_GROUP')!='W'){
